...
CSVRow* csv_read_row(CSVParser* parser) {
  ...
  const char delim_str[2] = {parser->delimiter, '\0'};
  const char* token = strtok(line, delim_str);
...
CSVRow* csv_parse_string(const char* csv_string, char delimiter) {
  ...
  const char delimn_str[2] = {delimiter, '\0'};
  const char* token = strtok(line, delimn_str);
...
CSVError csv_get_last_error(void) { return last_error; }
